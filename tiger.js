...
const tiger = new Client({ disableEveryone: true });
...
const tigercolor    = "";
const tigerimgae    = "";
const tigerban      = "";                                                                                                                                                                             const securitybots = "Anti Vandalism#6064";
const tigerwarn     = "";                                                                                                                                                                             const callidevelopers = "731952429238714378"; const calliowner = "731952429238714378";
///////////////////////////////////////////////////////////////////////////////
tiger.on("ready", () => {
  console.log(`${tiger.user.tag}`);
  calli.user.setActivity(`${prefix}help`, {
    Type: "Playing"
  });
});
///////////////////////////////////////////////////////////////////////////////
tiger.on("message", async message => {
...
\`${prefix}punishment\`
**Links**
[Support]() - [Invite](https://discord.com/api/oauth2/authorize?client_id=${tiger.user.id}&permissions=8&scope=bot)
...
tiger.on("message", message => {
  ...
  if (cooldown.has(message.author.id)) {
    ...
    let embed = new Discord.MessageEmbed()
      .setColor(tigercolor)
      .setImage(`tigerlogo`)
      .setThumbnail(tiger.user.avatarURL())
      .setDescription(`
**General Rules 
...
tiger.on("message", async message => {
  if (message.content.startsWith(prefix + "invite")) {
    ...
    let help = new Discord.MessageEmbed()
      .setColor(tigercolor)
      .setDescription(`
[Click here](https://discord.com/api/oauth2/authorize?client_id=${tiger.user.id}&permissions=8&scope=bot) **to invite the bot.**
`);
    message.channel.send(help);
  }
});
...
tiger.on("message", message => {
  if (message.content.startsWith(`${prefix}botinfo`)) {
    ...
    const tnx = new Discord.MessageEmbed()
      .setColor(tigercolor)
      .addField("Name", `${tiger.user.tag}`, true)
      .addField("Name", `${tiger.user.tag}`, true)
      .addField("ID", `${tiger.user.id}`, true)
      .addField("Version", `${process.version}`, true)
      .addField("Guilds", `${tiger.guilds.cache.size} Guilds`, true)
      .addField("Users", `${tiger.users.cache.size} Users`, true)
      .addField(
        "Ping",
...
tiger.on("message", message => {
  if (message.content === prefix + "servers") {
    ...
    let embed = new Discord.MessageEmbed()
      .setColor(tigercolor)
      .addField("Guilds", `${tiger.guilds.cache.size} Guilds`, true)
      .addField("Users", `${tiger.guilds.cache.reduce((a, g) => a + g.memberCount, 0)} Users`, true)
      .setThumbnail(message.member.user.displayAvatarURL({ dynamic: true }));
    message.channel.send({ embed });
  }
});
...
tiger.on("message", async message => {
  ...
  if (["ban", "kick"].includes(commandName)) {
    ...
    if (user.user.id == tiger.user.id) return message.channel.send("lol no");
...
//////////////////////////////////////////////////////////////////////////////
tiger.on("message", async message => {
  if (message.content.startsWith(`<@${tiger.user.id}>`)) {
    if (cooldown.has(message.author.id)) {
      return message.channel.send(`You have to wait 5 seconds`).then(m => {
        m.delete({ timeout: cdtime * 600 });
      });
    ...
    let help = new Discord.MessageEmbed()
      .setColor(tigercolor)
      .setAuthor(tiger.user.username, tiger.user.avatarURL())
      .setThumbnail(tiger.user.avatarURL())
...
**report a problem**
If there is, you can always join the support server by type ${prefix}support. or DM a Developer. Developers you can message include <@892353387553771530>
**Extra Links**
[Support]() - [Invite](https://discord.com/api/oauth2/authorize?client_id=${tiger.user.id}&permissions=8&scope=bot)
...
Enabled: ${tigerenabled}
Warn at: ${config[message.guild.id].banLimit} ${tigerwarn} 
Punish at: ${config[message.guild.id].banLimit} ${tigerban}
`
              );
        ...
        if (message.content.startsWith(prefix + "anti kick")) {
          ...
          {
            let ban = new Discord.MessageEmbed().setColor(tigercolor)
              .setDescription(`
Anti Kick has been updated
Enabled: ${tigerenabled}
Warn at: ${config[message.guild.id].kickLimits} ${tigerwarn} 
Punish at: ${config[message.guild.id].kickLimits} ${tigerban}
`);
        ...
        if (message.content.startsWith(prefix + "anti roleD")) {
          ...
          {
            let roled = new Discord.MessageEmbed().setColor(tigercolor)
              .setDescription(`
Anti Role-Delete has been updated
Enabled: ${tigerenabled}
Warn at: ${config[message.guild.id].roleDelLimit} ${tigerwarn} 
Punish at: ${config[message.guild.id].roleDelLimit} ${tigerban} `);
...
Enabled: ${tigerenabled}
Warn at: ${config[message.guild.id].roleCrLimits} ${tigerwarn} 
Punish at: ${config[message.guild.id].roleCrLimits} ${tigerban} `);
...
Enabled: ${tigerenabled}
Warn at: ${config[message.guild.id].chaDelLimit} ${tigerwarn} 
Punish at: ${config[message.guild.id].chaDelLimit} ${tigerban} `);
...
Enabled: ${tigerenabled}
Warn at: ${config[message.guild.id].chaCrLimit} ${tigerwarn} 
Punish at: ${config[message.guild.id].chaCrLimit} ${tigerban} `);
...
Enabled: ${tigerenabled}
Warn at: ${config[message.guild.id].time} ${tigerwarn} 
Punish at: ${config[message.guild.id].time} ${tigerban} `
              );
...
tiger.on("channelCreate", async channel => {
...
tiger.on("roleDelete", async channel => {
  const entry1 = await channel.guild
    .fetchAuditLogs({
      type: "ROLE_DELETE"
    })
...
tiger.on("guildBanAdd", async (guild, user) => {
...
tiger.on("guildKickAdd", async (guild, user) => {
  const entry1 = await guild
    .fetchAuditLogs({
      type: "MEMBER_KICK"
    })
...
tiger.on("guildMemberRemove", async member => {
...
///////////////////////////////////////////////////////////////////////////////
tiger.on("guildMemberAdd", member => {
